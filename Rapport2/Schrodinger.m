...
%Schrodinger(E,steps,s,color) plots the solution to the 
%Schrodinger equation, the input potential and also calculates the 
%conductance of the system. The program
...
%steps is recomended to be 0.1-0.01 any larger will increase the 
%calculation time without improving the calculations significantly.
%The value of s in the potential, since the default potential is 
%v=(beta-1/2)*Vsd+
%Vg*tanh(s*((intervalstart/step:intervallength/step-1)-x1/step))-
%(Vsd/2+Vg)*tanh(s*((intervalstart/step:intervallength/step-1)-x2/step));
...
acc=5;
boundryconditions=
[vpa(exp(1i*intervalstart*k),acc)+R*vpa(exp(-1i*intervalstart*k),acc),
    T*vpa(exp(1i*intervallength*q),acc)];
derivativeconditions=
[1i*k*exp(1i*intervalstart*k)-R*1i*k*exp(-1i*intervalstart*k),
    1i*q*T*exp(1i*intervallength*q)];
...
%v=0.5*tanh((intervalstart/step:intervallength/step-1)-4/step)-
%0.3*tanh((intervalstart/step:intervallength/step-1)-6/step);
v=(beta-1/2)*Vsd+
Vg*tanh(s*((intervalstart/step:intervallength/step-1)-x1/step))-
(Vsd/2+Vg)*tanh(s*((intervalstart/step:intervallength/step-1)-x2/step));
...
temp1=solve(answer(2)/step-answer(1)/step == derivativeconditions(1),
answer(size)/step-answer(size-1)/step == derivativeconditions(2),R,T);
